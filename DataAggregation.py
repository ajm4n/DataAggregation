...
def convert():
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        processed_tables = set()  # to keep track of processed tables
        for doc_path in doc_paths:
            ...
            for table in doc.tables:
                ...
                if ('Vulnerability' in headers and 'Root Cause' in headers and 'Rating' in headers
                        and table not in processed_tables):
                    for row in table.rows:
                        writer.writerow([cell.text for cell in row.cells])
                    processed_tables.add(table)
        result_label.config(text='Conversion complete!')# Create a label to show the result of the conversion
